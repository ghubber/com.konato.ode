...
(defn keepdecim
  ...
  [val digits]
  (/ (Math/round (* val (Math/pow 10.0 digits)))
...
(defn euler
  ...
  (into {} (for [[k fp] fps]
             [k (+ (k initvals)
                   (* stepsz (fp  (merge initvals {depvar depv0}))))])))


(defn eulerpc
  "Evaluate an ODE system using an improved Euler method using predictor and corrector."
  [fps initvals depvar depv0 stepsz]
  (let [dys  (into {} (for [[k fp] fps]
                        [k (fp  (merge initvals
                                       {depvar depv0}))]));
        y1p  (into {} (for [[k dy] dys]
                        [k (+ (k initvals) (* stepsz dy))]));y(n+1),p
        dy1p (into {} (for [[k fp] fps]
                        [k (fp (merge y1p
                                      {depvar (+ depv0 stepsz)}))]));dy(n+1),p
       ]
    (into {} (for [[k _] fps]
               [k (+ (k initvals)
                     (* stepsz (/ (+ (k dys) (k dy1p)) 2)))]))
))

(defn rk4
  "Evaluate an ODE system using an Runge-Kutta order 4."
  [fps initvals depvar depv0 stepsz]
  (let [k1 (into {} (for [[k fp] fps]
                      [k (* stepsz (fp (merge initvals
                                              {depvar depv0})))]))
        y1  (into {} (for [[k fp] fps]
                       [k (+ (k initvals)
                             (* 0.5 (k k1)))]))
        k2 (into {} (for [[k fp] fps]
                      [k (* stepsz
                            (fp (merge y1
                                       {depvar (+ depv0 (* 0.5 stepsz))})))]))
        y2  (into {} (for [[k fp] fps]
                       [k (+ (k initvals) (* 0.5 (k k2)))]))
        k3 (into {} (for [[k fp] fps]
                      [k (* stepsz (fp (merge y2
                                              {depvar (+ depv0
                                                         (* 0.5 stepsz))})))]))
        y3  (into {} (for [[k fp] fps]
                       [k (+ (k initvals) (k k3))]))
        k4 (into {} (for [[k fp] fps]
                      [k (* stepsz (fp (merge y3
                                              {depvar (+ depv0 stepsz)})))]))]
    (into {} (for [[k _] fps]
               [k (+ (k initvals)
                     (* 1/6 (+ (k k1)
...
(defn evalks
  ...
  (loop [alps alps
    ...
    (if (empty? alps)
      ...
      (let [
            yn (into {} (for [[k fp] fps]
                          [k (+ (k initvals)
                                (reduce + (map * (first bets) (k ks))))])) ;; compute yn+
            kn (into {} (for [[k fp] fps]
                          [k (* stepsz
                                (fp (merge yn
                                           {depvar (+ depv0
                                                      (* stepsz (first alps)))})))])) ;; compute kn+
            ks (into {} (for [[k rz] ks]
                          [k (concat rz (vector (k kn)))])) ; add k to list of ks
	    ]
...
(defn rkp
  ...
  (let [ks (evalks (:alphas par) (:betas par) fps initvals depvar depv0 stepsz);eval the ks
        rz  (into {} (for [[k fp] fps]
                       [k (+ (k initvals)
                             (* (first (:abs par))
                                (reduce + (map * (second (:abs par))
                                               (k ks)))))]));for each ode eval yn+1 = yn + abfact (ak1+bk2+...)
        err (if (:error par) (into [] (for [[k fp] fps]
                                        (Math/abs (reduce + (map * (:error par) (k ks)))))) 0.0);eval the error if :error param is present
     ]
...
(defn make-rkp [pars]
  ...
  (let [par pars]
    (fn  [fps initvals depvar depv0 stepsz]
      (rkp par fps initvals depvar depv0 stepsz))))

(def rkp4 (make-rkp {:abs [1/6 [1 2 2 1]],
                     :alphas [1/2 1/2 1],
                     :betas [[1/2][0 1/2][0 0 1]]}))

(def rkf45 (make-rkp
            {:abs [1 [16/135 0 6656/12825 28561/56430 -9/50 2/55]],
             :alphas [1/4 3/8 12/13 1 1/2],
             :betas [[1/4]
                     [3/32 9/32]
                     [1932/2197 -7200/2197 7296/2197]
                     [439/216 -8 3680/513 -845/4104]
                     [-8/27 2 -3544/2565 1859/4104 -11/40]],
             :error [1/360 0 -128/4275 -2197/75240 1/50 2/55]}))
...
(defmulti odesolve
  ...
  (fn [method fps initvals depvar depv0 depvf stepsz outfps]
    [(if (map? stepsz) :vstep :fstep)]))

;; Ordinary differential equations solver fixed step
(defmethod odesolve [:fstep]
  [method fps initvals depvar depv0 depvf stepsz outfps]
  "Solve an ODE system using a fixed method"
  (loop [tvals (take (Math/round (Math/abs (/ (- depvf depv0)
                                              stepsz)))
                     (interval-seq depv0 stepsz))
         rz (into {} (for [[k fp] outfps]
                       [k (vector (fp (merge initvals {depvar depv0})))])) ;eval the output fcts with initvals and depvar v0
                                        ;rz (conj (into {} (for [[k val] initvals] [k (vector val)])) {depvar (vector depv0)}) ;; initvals are included in the result
         ivals initvals]
    (if (empty? tvals)
      rz
      (let [vals1 (dissoc (method fps ivals depvar
                                  (first tvals) stepsz) :err)] ;eval and remove :err key not needed
        (recur
         (rest tvals)
         (into {} (for [[k fp] outfps]
                    [k (merge (k rz)
                              (fp (merge vals1
                                         {depvar (+ (first tvals) stepsz)})))]))
...
(defn- adjust-stepsz
  ...
  (let [s (if (= err 0) 0 (Math/pow (/ (* tol cursz)
                                       (* 2 err))
                                    1/4)) ;eval the new s
        ss (cond (< s 0.1) 0.1
                 (> s 4.0) 4.0
...
Map outfps allows selection of outputs, apply formulas."
  (loop [sz (:szstart stepsz)
         depvn depv0
         rz (into {} (for [[k fp] outfps]
                       [k (vector (fp (merge initvals
                                             {depvar depv0})))])) ;eval the output fcts with initvals and depvar v0
         ivals initvals
         iter 1]
    (if (or (>= (+ depvn (:eps stepsz)) depvf)
            (>= iter (:maxiter stepsz)))
      rz                                ;return the results
      (let [ev (method fps ivals depvar depvn sz)
            vals1 (dissoc ev :err)
            err (:err ev)]
        (if (and (>= err (:maxsteperr stepsz))
                 (>= sz (* 2 (:szmin stepsz))))
          (let [newsz (adjust-stepsz (:maxsteperr stepsz)
                                     err
                                     sz
                                     (:szmin stepsz)
                                     (:szmax stepsz))]
            (recur (if (> (+ depvn newsz) depvf)
                     (- depvf depvn) newsz)
                   depvn
                   rz
                   ivals
          ...
          (recur
           (if (> (+ depvn sz) depvf)
             (- depvf depvn) sz)     ; correct if > than end of interval
           (if (> (+ depvn sz) depvf)
             depvf (+ depvn sz))     ; correct if > than end of interval
           (into {} (for [[k fp] outfps]
                      [k (merge (k rz)
...
(defmacro frominput [ts us ivals]
  ...
  `(reduce + (map (fn [t# u#]
                    (* t# (cond (number? u#) u# :else (u# ~ivals))))
                  ~ts
                  ~us)))
...
(defn- tofps [prefixres prefix S T U]
  ...
  (into {} (map #(hash-map (keyword (str prefixres %1))
                           (fn [ival] (tofp prefix %2 %3 U ival)))
                (take (count S) (iterate inc 1)) S T)))
...
(defn tf2odedot1 [DEN]
  ...
  (into {} (map #(hash-map (keyword (str "x" %1))
                           (fn [ival] ((singledot (+ 1 %1)) ival)))
                (take (count (rest (rest DEN))) (iterate inc 1)))))
...
(defn tf2odedotlast [DEN U]
  ...
  (let [D (reverse (map #(* -1.0 %) (rest DEN)))]
    {(keyword (str "x" (int (- (count DEN) 1.0))))
     (fn [ival] (tofp "x" D [1] U ival))}))
...
(defn transfer2ode [NUM  DEN inp]
  ...
  (if (not (= 1 (- (count DEN) (count NUM))))
    ...
    (let [N (map #(/ % (first DEN)) NUM)
          D (map #(/ % (first DEN)) DEN)]
      {:dotfps (into (tf2odedot1 D) (tf2odedotlast D inp)),
       :outfps (tf2odeout N inp)})))
