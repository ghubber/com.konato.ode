...
"Test the Runge-Kutta order 4 method for a classic damped harmonic oscilator."
 (let [fps  {:x #(:xdot %), :xdot #(/ (+ (* -1.0 k (:x %)) (* -1.0 fv (:xdot %))) m)}
       outfps {:x #(:x %), :t #(:t %)}]
...
(comment
  ...
  (= 0 (reduce + (map #(- %1 %2)
                      (:x (dampedho 1 1 1 1.0 0.0 0.0 2.0 0.01))
                      (:y1 (transferdampedho 1 1 1 1.0 0.0 0.0 2.0 0.01)))));error study with transfer2ode conversion
  (= 0 (reduce +(map #(- %1 %2)
                     (:x (dampedho 1 1 1 1.0 0.0 0.0 2.0 0.01))
                     (:y1 (statedampedho 1 1 1 1.0 0.0 0.0 2.0 0.01)))));error study with state2ode conversion
)
